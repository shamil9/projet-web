...
var addFavorite = function(url, self) {
    var options = { url: url, method: "POST", context: self };
...
var showGoogleMap = function(address) {
    var map;
    map = new GMaps({ el: "#map", lat: -12.043333, lng: -77.028333 });
    GMaps.geocode({
        address: address,
    ...
    $.getJSON(url, function(data) {
        if (data.length < 1)
            return showSuggestions(
                '<h3 class="text-center">Aucun r\xE9sultat trouv\xE9</h3>'
            );
        var html = "";
        ...
        for (var key in data) {
            html += "<div class='col-lg-3'>" +
                ...
                data[key].slug +
                "' class='thumbnail'>" +
                "<img class='img--100x100' src='/assets/img/uploads/avatars/" +
                data[key].picture +
                "'>" +
...
var postComment = function(params) {
    ...
    $.ajax(options).done(function(data) {
        var comment = "<div class='panel panel-default comment-animate'>" +
            "<div class='panel-heading'>" +
            "<img class='img--30x30 img-circle' src='/assets/img/uploads/avatars/" +
            params.picture +
            "'/>" +
            "<span class='lead' style='vertical-align: middle;'>" +
            params.user +
            "</span>" +
            "<span class='pull-right rating' style='width: calc(20px * " +
            data.rating +
            ");'></span>" +
            "</div>" +
